...
类型：Informational（实现参考）

状态：v1 原始实现方案，核心切片已经落地

最近校验：2026-07-26

编制日期：2026-07-17

产品范围与行为以 [`product-design.md`](product-design.md) 为准。本文保留把产品基线转换为
架构、数据模型、模块边界、实现顺序和测试矩阵时的设计依据。文中的“当前实现”“需要替换”
和提交顺序描述的是方案制定时的基线，不用于判断今天的代码状态。
