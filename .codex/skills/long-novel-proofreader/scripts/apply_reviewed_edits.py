    ...
    try:
        ...
        if not state["ready_to_apply"]:
            raise ProofreadError(
                ...
                f"global_consistency={state['global_consistency_review']}"
            )
        ...
        for term in glossary.get("terms", []):
            if not term.get("protected"):
                continue
            protected = term["term"]
            authorized_reduction = sum(
                max(0, item["original"].count(protected) - item["final_proposed"].count(protected))
                for item in safe_edits
            )
            minimum_count = source_text.count(protected) - authorized_reduction
            if revised.count(protected) < minimum_count:
                raise ProofreadError(f"protected glossary term was changed without an approved edit: {protected}")
